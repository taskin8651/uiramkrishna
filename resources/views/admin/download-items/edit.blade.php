...
<div class="admin-page-head">
    <div>
        ...
        <h2 class="admin-page-title">Edit Download</h2>
        <p class="admin-page-subtitle">Update downloadable document or file.</p>
    </div>
</div>

<form method="POST"
      action="{{ route('admin.download-items.update', $downloadItem) }}"
      enctype="multipart/form-data">
    @csrf
    @method('PUT')

    @php
        $summaryItems = old('summary_items', $downloadItem->summary_items ?: array_fill(0, 3, ['label' => '', 'value' => '']));
        $summaryItems = array_pad($summaryItems, 3, ['label' => '', 'value' => '']);
        $infoCards = old('info_cards', $downloadItem->info_cards ?: array_fill(0, 4, ['icon' => '', 'title' => '', 'description' => '']));
        $infoCards = array_pad($infoCards, 4, ['icon' => '', 'title' => '', 'description' => '']);
        $tableRows = old('table_rows', $downloadItem->table_rows ?: array_fill(0, 3, ['title' => '', 'details' => '', 'button' => '']));
        $tableRows = array_pad($tableRows, 3, ['title' => '', 'details' => '', 'button' => '']);
    @endphp
            ...
            <div class="form-card-body">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="download_category_id"
                            ...
                            @foreach($categories as $id => $category)
                                <option value="{{ $id }}" {{ old('download_category_id', $downloadItem->download_category_id) == $id ? 'selected' : '' }}>
                                    {{ $category }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="field-group">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="5"
                              class="field-textarea">{{ old('description', $downloadItem->description) }}</textarea>
                </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text"
                               name="year"
                               id="year"
                               value="{{ old('year', $downloadItem->year) }}"
                               class="field-input">
                    </div>
                </div>
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div>
                    ...
                    <p class="form-card-subtitle">Replace or keep existing file</p>
                </div>
            </div>
            ...
            <div class="form-card-body">
                ...
                @if($downloadItem->download_file_url)
                    <div class="form-info-box" style="margin-bottom:16px;">
                        <p>
                            ...
                            Existing File:
                            <a href="{{ $downloadItem->download_file_url }}" target="_blank">
                                {{ $downloadItem->download_file_name ?? 'View File' }}
                            </a>
                ...
                <div class="field-group">
                    ...
                    @else
                        <p class="field-hint">
                            Upload new file only if you want to replace old file.
                        </p>
                    @endif
                </div>
                ...
                <label class="role-checkbox-item {{ old('status', $downloadItem->status) ? 'checked' : '' }}" style="margin-top:10px;">
                    <input type="checkbox"
                           name="status"
                           value="1"
                           class="role-checkbox"
            ...
            <div class="form-card-body">
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $downloadItem->slug) }}" class="field-input {{ $errors->has('slug') ? 'error' : '' }}">
                    </div>
                    @if($errors->has('slug'))
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text" name="kicker_text" id="kicker_text" value="{{ old('kicker_text', $downloadItem->kicker_text) }}" class="field-input">
                    </div>
                </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        <i class="fas fa-heading icon"></i>
                        <input type="text" name="hero_title" id="hero_title" value="{{ old('hero_title', $downloadItem->hero_title) }}" class="field-input">
                    </div>
                </div>

                <div class="field-group">
                    <label class="field-label" for="hero_description">Hero Description</label>
                    <textarea name="hero_description" id="hero_description" rows="4" class="field-textarea">{{ old('hero_description', $downloadItem->hero_description) }}</textarea>
                </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="url" name="external_url" id="external_url" value="{{ old('external_url', $downloadItem->external_url) }}" class="field-input {{ $errors->has('external_url') ? 'error' : '' }}">
                    </div>
                ...
                @foreach(['document_code' => 'Document Code', 'authority' => 'Authority', 'document_date' => 'Document Date', 'session_reference' => 'Session Reference', 'class_start' => 'Class Start'] as $field => $label)
                    <div class="field-group">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <input type="text" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $downloadItem->{$field}) }}" class="field-input">
                        </div>
                    </div>
                @endforeach
            </div>
