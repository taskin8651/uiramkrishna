...
@section('page-title', 'Edit Academic Info Page')

@section('content')

@php
    $cards = old('cards', $academicInfoPage->cards ?: array_fill(0, 3, ['icon' => '', 'title' => '', 'description' => '']));
    $cards = array_pad($cards, 3, ['icon' => '', 'title' => '', 'description' => '']);
    $tableRows = old('table_rows', $academicInfoPage->table_rows ?: array_fill(0, 3, ['title' => '', 'details' => '']));
    $tableRows = array_pad($tableRows, 3, ['title' => '', 'details' => '']);
@endphp
...
<div class="admin-page-head">
    <div>
        ...
        <h2 class="admin-page-title">Edit Academic Info Page</h2>
        <p class="admin-page-subtitle">Update academic information content.</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.academic-info-pages.update', $academicInfoPage) }}">
    @csrf
    @method('PUT')
            ...
            <div class="form-card-body">
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $academicInfoPage->slug) }}" required class="field-input {{ $errors->has('slug') ? 'error' : '' }}">
                    </div>
                    @if($errors->has('slug'))
                ...
                @foreach(['kicker_text' => 'Kicker Text', 'hero_title' => 'Hero Title'] as $field => $label)
                    <div class="field-group">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <input type="text" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $academicInfoPage->{$field}) }}" class="field-input">
                        </div>
                    </div>
                @endforeach
                ...
                <div class="field-group">
                    ...
                    <textarea name="hero_description" id="hero_description" rows="5" class="field-textarea">{{ old('hero_description', $academicInfoPage->hero_description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-card-body">
                ...
                <div class="field-group">
                    ...
                    <textarea name="section_description" id="section_description" rows="5" class="field-textarea">{{ old('section_description', $academicInfoPage->section_description) }}</textarea>
                </div>

                <label class="role-checkbox-item {{ old('status', $academicInfoPage->status) ? 'checked' : '' }}">
                    <input type="checkbox" name="status" value="1" class="role-checkbox" {{ old('status', $academicInfoPage->status) ? 'checked' : '' }}>
